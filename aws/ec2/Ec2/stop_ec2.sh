...
# Execute the AWS CLi to be able to list all of the associated ec2 instances with the above VPC
...
# Loop through all of the instances associated with the VPC and stop the respective Ec2 instances, excluding the Jenkins server
for ec2_id in $Instance_Ids; do
    if [ "$ec2_id" != "$Jenkins_EC2_ID" ]; then
        echo "Stopping the Dev Instance: $ec2_id"
        aws ec2 stop-instances --region "$AWS_Region" --instance-ids "$ec2_id"
    else
